...
const getAllCommentsForPost = async (req, res, next) => {
    const postId = req.params.id; // Id-ul postării pentru care dorim să obținem comentariile

    try {
        const post = await Post.findById(postId).populate('comments.author', 'name'); // Populăm câmpul 'author' al comentariilor cu numele autorului
        if (!post) {
            return next(new HttpError("Post not found.", 404));
        }
        
        const comments = post.comments; // Luăm lista de comentarii din postare
        
        res.status(200).json(comments); // Returnăm lista de comentarii
    } catch (error) {
        return next(new HttpError(error));
    }
};
...
const deleteComment = async (req, res, next) => {
    const postId = req.params.id;
    const commentId = req.params.commentId; // Assuming you pass commentId in the route
...
module.exports = { getPosts, getPost, getCatPosts, getUserPosts, createPost, editPost, removePost, addComment, deleteComment,getAllCommentsForPost };
